...
def rate_test():
    ...
    for r_test in r_values:
        ...
        bs_vals = bs_greeks(S0, K, r_test, sigma, T, is_call)
        data.append([
            r_test,
    ...
    for name, col_tree, col_bs, color, grid_x, grid_y in chart_specs:
        ...
        ax.plot(r_values * 100, bs_vals, color=color, linestyle="--", linewidth=1.5, label=f"{name} (BS)")
        ax.plot(r_values * 100, tree_vals, color=color, linewidth=1.5, label=f"{name} (Tree)")
