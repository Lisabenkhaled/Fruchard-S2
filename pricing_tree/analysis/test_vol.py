...
def compute_vol_prices(
    ...
    """Calcule les prix BS et arbre pour chaque niveau de volatilité."""
    bs_prices = []
    tree_prices = []

    # boucle sur les volatilités
    for vol in vol_values:
        market.sigma = vol

        # prix Black-Scholes
        bs_price_value = bs_price(s0, strike, rate, vol, maturity, is_call)
...
def test_vol() -> None:
    """Exécute le test de sensibilité à la volatilité."""
    (
        market,
        option,
        n_steps,
    ...
    # génération des volatilités de test
    vol_values = np.linspace(0.05, 0.5, 30)

    # calcul des prix
    bs_prices, tree_prices = compute_vol_prices(
        market,
        option,
        n_steps,
        exercise,
    ...
    diff = write_vol_results(sheet_pr, vol_values, bs_prices, tree_prices)

    # création du graphique
    add_vol_chart(sheet_pr, vol_values, bs_prices, tree_prices, diff)


def run_volatility_test() -> None:
    """Lance le test de volatilité."""
    test_vol()
