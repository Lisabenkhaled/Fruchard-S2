...
def greek_wrapper(params: tuple[Any, Any, int, str, str | bool, 
                                float, str, str], x: float) -> float:
    """
...
def finite_diff_2d(
    market: Any,option: Any,N: int,exercise: str,
    optimize: str | bool,threshold: float,
    method: str,base_price: float) -> tuple[float, float]:
    
    S0, sigma0 = market.S0, market.sigma;hS = max(1e-5, 0.01 * S0)
    hSigma = max(1e-5, 0.005)

    def price_shift(dS: float = 0.0, dSigma: float = 0.0) -> float:
        m = copy.deepcopy(market);m.S0 = S0 + dS
        m.sigma = max(1e-6, sigma0 + dSigma)
        return get_price(m, option, N, exercise, optimize, threshold, method)

    # Calcul des dérivées croisées
    p_up_up     = price_shift(+hS, +hSigma);p_up_down   = price_shift(+hS, -hSigma)
    p_down_up   = price_shift(-hS, +hSigma);p_down_down = price_shift(-hS, -hSigma)
...
def compute_method_greeks(
    ...
    hS = max(1e-5, 0.005 * market.S0);hSigma = max(1e-5, 0.005)
    hR = 1e-4;hT = 1.0 / 365.0  # un jour
    ...
    # Calcul des grecs
    Delta = dS.first(market.S0);Gamma = dS.second(market.S0)
    Vega  = dSigma.first(market.sigma);Rho   = dR.first(market.r)
...
def compute_greeks() -> dict[str, dict[str, float]]:
    """
    ...
    ws = ensure_sheet(wb, "Greeks")

    # Nettoyage de la feuille
    ws.range((6, 3), (ws.cells.last_cell.row, ws.cells.last_cell.column)).clear_contents()
