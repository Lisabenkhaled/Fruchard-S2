...
def volatility_test():
    ...
    for name, col_tree, col_bs, color, grid_x, grid_y in chart_specs:
        ...
        ax.plot(np.array(vol_values) * 100, bs_vals, color=color, linestyle="--", linewidth=1.5, label=f"{name} (BS)")
        ax.plot(np.array(vol_values) * 100, tree_vals, color=color, linewidth=1.5, label=f"{name} (Tree)")
...
def run_test_greeks_volatility():
    volatility_test()
