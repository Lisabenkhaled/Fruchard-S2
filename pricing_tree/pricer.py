...
def price_tree_backward_direct(
    *,S0: float,r: float,sigma: float,K: float,is_call: bool,
    exercise: Exercise,pricing_date: dt.date,maturity_date: dt.date,
    N: int = 10_000,ex_div_date: dt.date | None = None,
    div_amount: float = 0.0,optimize: bool = False,
    threshold: float = 1e-14,return_tree: bool = False) -> dict[str, Any]:
    """
    ...
    market = Market(S0=float(S0),r=float(r),sigma=float(sigma),
        T=float(T),exdivdate=exdivdate,pricing_date=None,
    ...
    tree = TrinomialTree(market, option, int(N), exercise);tree.build_tree()
    tree.compute_reach_probabilities()

    if optimize:
        tree.prune_tree(float(threshold))
    # price and time
    price = float(price_backward(tree));elapsed = float(time.time() - t0)

    out: Dict[str, Any] = {"tree_price": price,"tree_time": elapsed}
    if return_tree:
        out["tree"] = tree
    return out
